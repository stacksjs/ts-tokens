...
export function NFTCard({ mint, showDetails = false, className, style }: NFTProps): JSX.Element {
  ...
  if (loading) return <div className={className} style={style} role="status" aria-label="Loading NFT">Loading...</div>
  if (error) return <div className={className} style={style} role="alert">Error loading NFT</div>
