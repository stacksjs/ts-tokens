...
export function serializeGuardSet(guards: GuardSet): Buffer {
  ...
  let features = 0n
  if (guards.botTax) features |= 1n << BigInt(GuardType.BotTax)
  if (guards.solPayment) features |= 1n << BigInt(GuardType.SolPayment)
  if (guards.tokenPayment) features |= 1n << BigInt(GuardType.TokenPayment)
  if (guards.startDate) features |= 1n << BigInt(GuardType.StartDate)
  if (guards.thirdPartySigner) features |= 1n << BigInt(GuardType.ThirdPartySigner)
  if (guards.tokenGate) features |= 1n << BigInt(GuardType.TokenGate)
  if (guards.gatekeeper) features |= 1n << BigInt(GuardType.Gatekeeper)
  if (guards.endDate) features |= 1n << BigInt(GuardType.EndDate)
  if (guards.allowList) features |= 1n << BigInt(GuardType.AllowList)
  if (guards.mintLimit) features |= 1n << BigInt(GuardType.MintLimit)
  if (guards.nftPayment) features |= 1n << BigInt(GuardType.NftPayment)
  if (guards.redeemedAmount) features |= 1n << BigInt(GuardType.RedeemedAmount)
  if (guards.addressGate) features |= 1n << BigInt(GuardType.AddressGate)
  if (guards.nftGate) features |= 1n << BigInt(GuardType.NftGate)
  if (guards.nftBurn) features |= 1n << BigInt(GuardType.NftBurn)
  if (guards.tokenBurn) features |= 1n << BigInt(GuardType.TokenBurn)
  if (guards.freezeSolPayment) features |= 1n << BigInt(GuardType.FreezeSolPayment)
  if (guards.freezeTokenPayment) features |= 1n << BigInt(GuardType.FreezeTokenPayment)
  if (guards.programGate) features |= 1n << BigInt(GuardType.ProgramGate)
  if (guards.allocation) features |= 1n << BigInt(GuardType.Allocation)
  if (guards.token2022Payment) features |= 1n << BigInt(GuardType.Token2022Payment)
  ...
  if (guards.botTax) parts.push(serializeBotTax(guards.botTax))
  if (guards.solPayment) parts.push(serializeSolPayment(guards.solPayment))
  if (guards.tokenPayment) parts.push(serializeTokenPayment(guards.tokenPayment))
  if (guards.startDate) parts.push(serializeStartDate(guards.startDate))
  if (guards.thirdPartySigner) parts.push(serializeThirdPartySigner(guards.thirdPartySigner))
  if (guards.tokenGate) parts.push(serializeTokenGate(guards.tokenGate))
  if (guards.gatekeeper) parts.push(serializeGatekeeper(guards.gatekeeper))
  if (guards.endDate) parts.push(serializeEndDate(guards.endDate))
  if (guards.allowList) parts.push(serializeAllowList(guards.allowList))
  if (guards.mintLimit) parts.push(serializeMintLimit(guards.mintLimit))
  if (guards.nftPayment) parts.push(serializeNftPayment(guards.nftPayment))
  if (guards.redeemedAmount) parts.push(serializeRedeemedAmount(guards.redeemedAmount))
  if (guards.addressGate) parts.push(serializeAddressGate(guards.addressGate))
  if (guards.nftGate) parts.push(serializeNftGate(guards.nftGate))
  if (guards.nftBurn) parts.push(serializeNftBurn(guards.nftBurn))
  if (guards.tokenBurn) parts.push(serializeTokenBurn(guards.tokenBurn))
  if (guards.freezeSolPayment) parts.push(serializeFreezeSolPayment(guards.freezeSolPayment))
  if (guards.freezeTokenPayment) parts.push(serializeFreezeTokenPayment(guards.freezeTokenPayment))
  if (guards.programGate) parts.push(serializeProgramGate(guards.programGate))
  if (guards.allocation) parts.push(serializeAllocation(guards.allocation))
  if (guards.token2022Payment) parts.push(serializeToken2022Payment(guards.token2022Payment))
