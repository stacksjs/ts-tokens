...
export const governanceCommands = {
  ...
  'dao:create': {
    ...
    options: [
      ...
      { name: 'vote-weight', type: 'string', default: 'token', description: 'Vote weight type: token, quadratic, nft, time-weighted' },
      { name: 'allow-early-execution', type: 'boolean', default: false, description: 'Allow early execution' },
      { name: 'allow-vote-change', type: 'boolean', default: false, description: 'Allow vote changes' },
    ],
  },
  ...
  'dao:config': {
    ...
    options: [
      ...
      { name: 'voting-period', type: 'string', description: 'New voting period' },
    ],
  },
  ...
  'delegate': {
    ...
    options: [
      ...
      { name: 'expires', type: 'string', description: 'Expiration time (e.g., "30 days")' },
    ],
  },
...
export function formatDAOInfo(dao: {
  ...
  const lines = [
    ...
    `  Vote Weight: ${dao.config.voteWeightType ?? 'token'}`,
    `  Early Execution: ${dao.config.allowEarlyExecution ?? false}`,
    `  Vote Changes: ${dao.config.allowVoteChange ?? false}`,
    '',
